...
public:
    PurePursuitTest(const Path& robot_path, const double& lookahead_distance): PurePursuit(robot_path, lookahead_distance) {}
    Point2D get_target_state_test(const Point3D& state)
    {
        return get_target_state(state);
    }
    void reset_path_test(const Path& robot_path)
    {
        reset_path(robot_path);
    }
    ...
    std::pair<Point2D,Point2D> get_ongoing_path_segment_test(const int& current_segment)
    {
