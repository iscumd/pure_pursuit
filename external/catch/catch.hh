...
#define CATCH_CONFIG_COUNTER
    ...
    && !defined( CATCH_CONFIG_NO_WINDOWS_SEH ) && !defined( CATCH_CONFIG_WINDOWS_SEH )
...
#endif
// This is set by default, because we assume that compilers with no wchar_t support are
...
inline auto operator"" _sr( char const* rawChars, std::size_t size ) noexcept
    -> StringRef
{
...
// Import begin/ end from std here so they are considered alongside the fallback (...)
// overloads in this namespace
using std::begin;
using std::end;
...
struct StringMaker<R, typename std::enable_if<is_range<R>::value
                                              && !::Catch::Detail::
                                                     IsStreamInsertable<R>::value>::type>
{
...
#pragma warning( disable : 4312 )  // Converting int to T* using reinterpret_cast (issue
                                   // on x64 platform)
...
// Specialised comparison functions to handle equality comparisons between ints and
// pointers (NULL deduces as an int)
...
public:
    AssertionHandler( StringRef macroName, SourceLineInfo const& lineInfo,
                      StringRef capturedExpression,
                      ResultDisposition::Flags resultDisposition );
    ~AssertionHandler()
    {
...
#define INTERNAL_CATCH_TEST( macroName, resultDisposition, ... )                        \
    ...
    {                                                                                   \
        ...
        INTERNAL_CATCH_REACT( catchAssertionHandler )                                   \
              ...
              false && static_cast<bool>( !!( __VA_ARGS__ ) ) )  // the expression here
                                                                 // is never evaluated at
                                                                 // runtime but it forces
                                                                 // the compiler to give
                                                                 // it a look
...
struct SectionEndInfo
{
    SectionEndInfo( SectionInfo const& _sectionInfo, Counts const& _prevAssertions,
...
namespace Detail
{
    ...
    public:
        ...
        Approx& margin( T const& newMargin )
        {
            ...
            if ( marginAsDouble < 0 )
            {
                throw std::domain_error( "Invalid Approx::margin: "
...
namespace Matchers
{
    namespace Impl
    {
        ...
        template <typename PtrT> struct MatcherMethod<PtrT*>
        {
    ...
    namespace Vector
    {
        ...
        template <typename T> struct UnorderedEqualsMatcher : MatcherBase<std::vector<T>>
        {
            ...
            bool match( std::vector<T> const& vec ) const override
            {
                ...
                //       because I don't want to include <algorithm> inside the common
                //       path
...
auto makeMatchExpr( ArgT const& arg, MatcherT const& matcher, StringRef matcherString )
    ...
    {                                                                                   \
        ...
        {                                                                               \
            catchAssertionHandler.handleExpr(                                           \
                Catch::makeMatchExpr( arg, matcher, #matcher ) );                       \
        }                                                                               \
        INTERNAL_CATCH_CATCH( catchAssertionHandler )                                   \
        INTERNAL_CATCH_REACT( catchAssertionHandler )                                   \
    } while ( false )
...
#define INTERNAL_CATCH_THROWS_MATCHES( macroName, exceptionType, resultDisposition,     \
    ...
    {                                                                                   \
        ...
        if ( catchAssertionHandler.allowThrows() )                                      \
            ...
            {                                                                           \
                catchAssertionHandler.handleExpr(                                       \
                    Catch::makeMatchExpr( ex, matcher, #matcher ) );                    \
            }                                                                           \
            catch ( ... )                                                               \
            {                                                                           \
                catchAssertionHandler.handleUnexpectedInflightException();              \
    ...
    namespace Impl
    {
        namespace NSStringMatchers
        {
            ...
            struct StringHolder : MatcherBase<NSString*>
            {
                ...
                bool match( NSString* arg ) const override { return false; }
...
private:
    ...
    template <typename T> void addPattern()
    {
        ...
        for ( std::size_t i = 0; i < m_escapeChars.size(); ++i )
            token = token.substr( 0, m_escapeChars[i] - m_start - i )
                + token.substr( m_escapeChars[i] - m_start - i + 1 );
        m_escapeChars.clear();
...
void addReporter( IStreamingReporterPtr& existingReporter,
                  IStreamingReporterPtr&& additionalReporter );
...
public:
    explicit ReporterRegistrar( std::string const& name )
    {
        getMutableRegistryHub().registerReporter( name,
                                                  std::make_shared<ReporterFactory>() );
    }
};
...
namespace TestCaseTracking
{
    ...
    public:
        IndexTracker( NameAndLocation const& nameAndLocation, TrackerContext& ctx,
        ...
        static IndexTracker& acquire( TrackerContext& ctx,
                                      NameAndLocation const& nameAndLocation, int size );
...
namespace Detail
{
    ...
    {
        // First try with fixed margin, then compute margin based on epsilon, scale and
        // Approx's value
...
AssertionHandler::AssertionHandler( StringRef macroName, SourceLineInfo const& lineInfo,
                                    StringRef capturedExpression,
                                    ResultDisposition::Flags resultDisposition )
...
// The more general overload, that takes any string matcher, is in
...
{
    return getEstimatedClockResolution()
        * getCurrentContext().getConfig()->benchmarkResolutionMultiple();
}
...
// There is another overload, in catch_assertionhandler.h/.cpp, that only takes a string
// and infers
...
// See the accompanying LICENSE file, or the one at
            ...
            public:
                ...
                auto operator*() const -> std::string
                {
                    ...
                    if ( m_pos + m_column.m_width < m_end )
                        ...
                        return addIndentAndSuffix(
                            line().substr( m_pos, m_end - m_pos ) );
                }
...
namespace clara
{
    namespace detail
    {
        ...
        // Wraps a token coming from a token stream. These may not directly correspond to
        ...
        template <typename L> struct BoundLambda : BoundValueRefBase
        {
            ...
            auto setValue( std::string const& arg ) -> ParserResult override
            {
                return invokeLambda<typename UnaryLambdaTraits<L>::ArgType>( m_lambda,
                                                                             arg );
            }
        };
...
clara::Parser makeCommandLineParser( ConfigData& config )
{
    ...
    auto cli = ExeName( config.processName ) | Help( config.showHelp )
        ...
        | Opt( setWaitForKeypress, "start|exit|both" )["--wait-for-keypress"](
                   "waits for a keypress before exiting" )
        | Opt( config.benchmarkResolutionMultiple,
               "multiplier" )["--benchmark-resolution-multiple"](
...
{
    return line < other.line
        || ( line == other.line && ( std::strcmp( file, other.file ) < 0 ) );
}
...
#define CATCH_INTERNAL_ERROR( msg )                                                     \
    throw CATCH_PREPARE_EXCEPTION( std::logic_error,                                    \
                                   CATCH_INTERNAL_LINEINFO                              \
                                       << ": Internal Catch error: " << msg );
...
std::vector<std::string> const& Config::getReporterNames() const
{
...
public:  // IContext
    ...
    virtual IConfigPtr const& getConfig() const override { return m_config; }
...
void FatalConditionHandler::reset()
{
    if ( isSet )
    {
        // Set signals back to previous values -- hopefully nobody overwrote them in the
        // meantime
        for ( std::size_t i = 0; i < sizeof( signalDefs ) / sizeof( SignalDefs ); ++i )
        {
...
TestGroupStats::TestGroupStats( GroupInfo const& _groupInfo, Totals const& _totals,
                                bool _aborting )
    ...
    for ( auto const& testCase : matchedTestCases )
    {
        for ( auto const& tagName : testCase.getTestCaseInfo().tags )
        {
            ...
            if ( countIt == tagCounts.end() )
                countIt = tagCounts.insert( std::make_pair( lcaseTagName, TagInfo() ) )
                              .first;
...
namespace Matchers
{
    namespace Floating
    {
        ...
        WithinUlpsMatcher::WithinUlpsMatcher( double target, int ulps,
        ...
        {
            return "is within " + std::to_string( m_ulps ) + " ULPs of "
                + ::Catch::Detail::stringify( m_target )
...
MessageInfo::MessageInfo( std::string const& _macroName, SourceLineInfo const& _lineInfo,
                          ResultWas::OfType _type )
...
public:
    ...
    ~RedirectedStream() { m_originalStream.rdbuf( m_prevBuf ); }
};
...
void RunContext::assertionEnded( AssertionResult const& result )
{
    ...
    // We have no use for the return value (whether messages should be cleared), because
    // messages were made scoped
...
SectionEndInfo::SectionEndInfo( SectionInfo const& _sectionInfo,
...
void StartupExceptionRegistry::add( std::exception_ptr const& exception ) noexcept
{
    ...
    {
        // If we run out of memory during start-up there's really not a lot more we can
        // do about it
        std::terminate();
    }
}
...
void ReusableStringStream::cleanup() { StringStreams::cleanup(); }
                                                ...
                                                SourceLineInfo const& lineInfo )
{
    ...
    {
        // Do not throw when constructing global objects, instead register the exception
...
namespace TestCaseTracking
{
    ...
    ITrackerPtr TrackerBase::findChild( NameAndLocation const& nameAndLocation )
    {
        auto it = std::find_if( m_children.begin(), m_children.end(),
                                TrackerHasName( nameAndLocation ) );
    ...
    {
        if ( !isComplete() && ( m_filters.empty() || m_filters[0].empty()
                                || m_filters[0] == m_nameAndLocation.name ) )
            open();
    }
    ...
    IndexTracker::IndexTracker( NameAndLocation const& nameAndLocation,
                                TrackerContext& ctx, ITracker* parent, int size )
        : TrackerBase( nameAndLocation, ctx, parent )
        , m_size( size )
    {
    }
    ...
    IndexTracker& IndexTracker::acquire( TrackerContext& ctx,
                                         NameAndLocation const& nameAndLocation,
                                         int size )
    {
        ...
        {
            tracker = std::make_shared<IndexTracker>( nameAndLocation, ctx,
                                                      &currentTracker, size );
...
auto estimateClockResolution() -> uint64_t
{
    ...
    // We're just taking the mean, here. To do better we could take the std. dev and
    // exclude outliers
...
void ConsoleReporter::benchmarkEnded( BenchmarkStats const& stats )
{
    ...
    ( *m_tablePrinter ) << stats.iterations << ColumnBreak()
                        << stats.elapsedTimeInNanoseconds << ColumnBreak() << average
                        << ColumnBreak();
}
...
namespace
{
    ...
    {
        auto it
            = std::find_if( begin( tags ), end( tags ), []( std::string const& tag ) {
                  return tag.front() == '#';
              } );
        if ( it != tags.end() )
            return it->substr( 1 );
...
{
    return "Reports test results in an XML format that looks like Ant's junitreport "
           "target";
}
...
#define CATCH_REQUIRE_THROWS( ... )                                                     \
    INTERNAL_CATCH_THROWS( "CATCH_REQUIRE_THROWS", Catch::ResultDisposition::Normal,    \
                           "", __VA_ARGS__ )
...
#define CATCH_CHECK( ... )                                                              \
    INTERNAL_CATCH_TEST( "CATCH_CHECK", Catch::ResultDisposition::ContinueOnFailure,    \
                         __VA_ARGS__ )
...
#define CATCH_CAPTURE( msg )                                                            \
    INTERNAL_CATCH_INFO( "CATCH_CAPTURE",                                               \
                         #msg " := " << ::Catch::Detail::stringify( msg ) )
